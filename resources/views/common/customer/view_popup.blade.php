                                        ...
                                        <form action="#">
                                            ...
                                            <div class="d-flex mb-10">
                                                @if(auth()->guard('customer')->check() && session()->has('authenticate_user') && session('authenticate_user')->store_id == $store_id)
                                                    <button type="button" title="{{ __('customer.add_to_wishlist') }}" data-page="" class="product-wishlist no-margin-left"><i data-wishlist-type="add" class="wishlist-icon far fa-heart"></i> {{ __('customer.add_to_wishlist') }}</button>
                                                @endif
                                                <button type="button" data-type="product-in-popup" class="product-add-to-cart add-to-cart{{ !(auth()->guard('customer')->check() && session()->has('authenticate_user') && session('authenticate_user')->store_id == $store_id) ? ' no-margin-left' : '' }}">
