                            ...
                            @if(isset($product_details) && !empty($product_details) && count($product_details) > 0)
                                @foreach ($product_details as $key => $product)
                                    ...
                                    @if($product->type_of_product == "variant")
                                        <div class="{{ ($product->product_available == 'out-of-stock') ? 'out-of-stock-cart' : '' }} product-cart-list product-cart-list-{{ $product->product_id }}-{{ $product->variants_combination_id }}" data-element="product-cart-list-{{ $product->product_id }}-{{ $product->variants_combination_id }}">
                                            ...
                                            @elseif($product->on_hand != "" && $product->on_hand < $get_quantity[$product->product_id][$product->variants_combination_id])
                                                <div class="mb-15 unavailable-error-message"> 
                                                    <span class="text-danger">{{ __('customer.quantity_adjusted',['on_hand' => $product->on_hand]) }}</span>
                                                </div>
                                            @endif
                                            <div class="d-flex gap-3 justify-content-between mb-3">
                                                ...
                                                <input type="hidden" class="variants-combination-json variant-combinations-{{ $product->product_id }}" value="{{ (isset($variants_combinations_data) && !empty($variants_combinations_data) && isset($variants_combinations_data[$product->product_id]) ? json_encode($variants_combinations_data[$product->product_id]) : '') }}">
                                                <div class="d-flex gap-3">
                                                    <div class="">
                                                        ...
                                                        @if(count($category_image) > 0)
                                                            <img style="width: 70px;" src="{{ $category_image[0] }}" alt="Product Image">
                                                        @endif
                                                    </div>
                                                    <div class="">
                                                        <p class="mb-2 fw-bold"><a href="{{ route($store_url . '.customer.single-product', ['id' => Crypt::encrypt($product->product_id), 'type' => Crypt::encrypt('product_page')]) }}" class="product-name">{{ $product['product_name'] }}</a></p>
                                                        <p class="fw-bolder mb-2 single-product-price">SAR {{ number_format((float)($product->variant_price), 2, '.', '') }}</p>
                                                        <input type="hidden" class="variants-title" value="{{ (isset($product_variants_title) && !empty($product_variants_title) && isset($product_variants_title[$product->product_id])) ? $product_variants_title[$product->product_id] : '' }}">                                                   
                                                        @if($product->product_available == "out-of-stock")
                                                            <p class="mb-0">
                                                                {{ (isset($product_variants_title) && !empty($product_variants_title) && isset($product_variants_title[$product->product_id])) ? $product_variants_title[$product->product_id] : "" }} : {{ $product->variants_combination_name }}
                                                            </p>
                                                        @else
                                                            <p class="mb-0 cart-variants" data-bs-toggle="modal" data-bs-target="#cart-variants-modal">
                                                                {{ (isset($product_variants_title) && !empty($product_variants_title) && isset($product_variants_title[$product->product_id])) ? $product_variants_title[$product->product_id] : "" }} : {{ $product->variants_combination_name }} <i class="fa fa-chevron-down" aria-hidden="true"></i>
                                                            </p> 
                                                        @endif
                                                    </div>
                                                </div>
                                                    ...
                                                    <div class="product_pro_button quantity1 mb-3">
                                                        @if($product->product_available != "out-of-stock")
                                                            <div class="pro-qty border number product-item">
                                                                <input type="hidden" class="product-price" value="{{ $product->variant_price }}">
                                                                <input type="hidden" class="product-unit-price" value="{{ $product->variant_price }}">
                                                                <input type="hidden" class="product-quantity" value="{{ $get_quantity[$product->product_id][$product->variants_combination_id] }}">
                                                                ...
                                                                <input type="hidden" class="get-product-quantity" value="{{ $product->variant_quantity }}">
                                                                ...
                                                                <input type="text" name="product_item[{{$product->category_id}}][{{$product->product_id}}][]" value="{{ ($product->on_hand != '' && $product->on_hand < $get_quantity[$product->product_id][$product->variants_combination_id]) ? $product->on_hand :  $get_quantity[$product->product_id][$product->variants_combination_id] }}" class="quantity" onkeypress="return isNumber(event)" style="height: 35px;">
                                                                <input type="hidden" name="product_amount[{{$product->category_id}}][{{$product->product_id}}][]" class="total-product-price" value="{{ $get_quantity[$product->product_id][$product->variants_combination_id] * $product->variant_price }}">
                                                    ...
                                                    <ul class="d-flex justify-content-center mb-0"> 
                                                        <li class="delete-product-item mr-30">
                                                            <a href="#01" title="Delete" class="text-danger mb-0">
                                                                {!! ($product->product_available == "out-of-stock") ? __('customer.remove_from_bag') : '<i class="fa fa-trash-o"></i> '.__('customer.delete') !!}
                                                            </a>
                                                        </li>
                                                        @if((auth()->guard('customer')->check() && session()->has('authenticate_user') && session('authenticate_user')->store_id == $store_id))
                                                            <li class="wishlist move-to-wishlist"><a href="#01" title="{{ __('customer.move_to_wishlist') }}" class="text-primary mb-0"><i data-wishlist-type="add" class="wishlist-icon fa fa-heart-o"></i> {{ __('customer.move_to_wishlist') }}</a></li> 
                                                        @endif
                                                    </ul>
                                                </div>
                                            </div>
                                    ...
                                    @else
                                        <div class="{{ ($product->trackable == 1 && $product->unit <= 0) ? 'out-of-stock-cart' : '' }} product-cart-list">
                                            @if($product->trackable == 1 && $product->unit <= 0)
                                                <div class="mb-15">
                                                    <span class="text-danger"><b><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> {{ __('customer.out_of_stock') }}</b></span><br/>
                                                    <span>{{ __('customer.unavailable_products') }}</span>
                                                </div>
                                            @elseif($product->trackable == 1 && $product->unit < $get_quantity[$product->product_id])
                                                <div class="mb-15 unavailable-error-message"> 
                                                    <span class="text-danger">{{ __('customer.quantity_adjusted',['on_hand' => $product->unit]) }}</span>
                                                </div>
                                            @endif
                                            <div class="d-flex gap-3 justify-content-between mb-3">
                                                <input type="hidden" class="product-name" value="{{ $product->product_name }}"> 
                                                <div class="d-flex gap-3">
                                                    <div class="">
                                                        ...
                                                        @endphp
                                                        @if(count($category_image) > 0)
                                                            <img style="width: 70px;" src="{{ $category_image[0] }}" alt="Cart Thumbnail">
                                                        @endif
                                                    </div>
                                                    <div class="">
                                                        <p class="mb-2 fw-bold"><a href="{{ route($store_url . '.customer.single-product', ['id' => Crypt::encrypt($product->product_id), 'type' => Crypt::encrypt('product_page')]) }}" class="product-name">{{ $product['product_name'] }}</a></p>
                                                        <p class="fw-bolder mb-2 single-product-price">SAR {{ number_format((float)($product->price), 2, '.', '') }}</p>
                                                    </div>
                                                </div>
                                                <div class="text-end">
                                                    <div class="product_pro_button quantity1 mb-3">
                                                        @if(($product->trackable == 1 && $product->unit > 0) || $product->trackable == 0)
                                                            <div class="pro-qty border number product-item">
                                                                ...
                                                                <input type="hidden" class="product-unit" value="{{ $product->unit }}"> 
                                                                <input type="hidden" class="type-of-product" value="{{ $product->type_of_product }}">
                                                                <input type="text" name="product_item[{{$product->category_id}}][{{$product->product_id}}][]" value="{{ ($product->trackable == 1 && $product->unit < $get_quantity[$product->product_id]) ? $product->unit : $get_quantity[$product->product_id] }}" class="quantity" onkeypress="return isNumber(event)" style="height: 35px;">
                                                    ...
                                                    <ul class="d-flex justify-content-center mb-0"> 
                                                        <li class="delete-product-item mr-30">
                                                            <a href="#01" title="Delete" class="text-danger mb-0">
                                                                {!! ($product->trackable == 1 && $product->unit <= 0) ? __('customer.remove_from_bag') : '<i class="fa fa-trash-o"></i> '.__('customer.delete') !!}
                                                            </a>
                                                        </li>
                                                        @if((auth()->guard('customer')->check() && session()->has('authenticate_user') && session('authenticate_user')->store_id == $store_id))
                                                            <li class="wishlist move-to-wishlist"><a href="#01" title="{{ __('customer.move_to_wishlist') }}" class="text-primary mb-0"><i data-wishlist-type="add" class="wishlist-icon fa fa-heart-o"></i> {{ __('customer.move_to_wishlist') }}</a></li> 
            ...
            <div class="modal-dialog modal-sm custom-modal product-cart-list single-product-details">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <span><b>{{ __('customer.select_quantity') }}</b></span>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-dark w-100 cart-update">{{ __('customer.update') }}</button>
                    </div>
                </div>
            </div>
        </div>
