...
class OnlineOrderStatusController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('cashier_admin.onine_order_status.list',compact('store_url','online_order_status','store_logo'));
    }
        ...
        if(!empty($add_status_order)) {
            for($i = 0;$i<count($add_status_order);$i++) {
                ...
                if(!empty($status_id[$i])) {
                    OnlineOrderStatus::where([ 
                        ['store_id','=',Auth::user()->store_id],
                        ['order_status_id','=',Crypt::decrypt($status_id[$i])],
                    ])->update($status_data);
                } else {
                    $status_data['created_by'] = Auth::user()->id;
                    $status_data['store_id'] = Auth::user()->store_id;
                    OnlineOrderStatus::create($status_data);
                }
            }
            return redirect()->route(config('app.prefix_url').'.'.$this->store_url.'.'.$this->prefix_url.'.online-order-status.index')->with('message',trans('store-admin.updated_msg',['name'=>trans('store-admin.online_order_status')]));
        } else {
            return redirect()->route(config('app.prefix_url').'.'.$this->store_url.'.'.$this->prefix_url.'.online-order-status.index')->with('error',trans('store-admin.no_records_updated_error'));
        }
    }
    ...
    public function destroy(Request $request)
    {
        ...
        return response()->json(['message'=>trans('store-admin.deleted_msg',['name'=>trans('store-admin.online_order_status')])]);
    }
}
